...
def main(total_games, num_games_per_file, save_dir, num_processes):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_processes) as executor:
        ...
        # Submit full runs
        for i in range(runs):
            futures.append(executor.submit(run_othello, num_games_per_file, save_dir, i))
        # If there's a remainder, submit one more run
        if remainder > 0:
            futures.append(executor.submit(run_othello, remainder, save_dir, runs))
...
if __name__ == "__main__":
    ...
    main(args.total_games, args.num_games_per_file, args.dir, args.num_proc)